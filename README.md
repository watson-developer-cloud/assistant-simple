...
1. Paste  the `password` and `username` values (without quotation marks) from the JSON into the `WATSON_ASSISTANT_PASSWORD` and `WATSON_ASSISTANT_USERNAME` variables in the `.env` file. For example:
